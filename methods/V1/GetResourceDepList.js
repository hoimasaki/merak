...
module.exports = ApiMethod.extend({
    process: function () {
    	...
    	var avg_sql = util.format("SELECT COUNT(DISTINCT instance_name) AS count , owt AS dep,\
          ...
          FROM ( SELECT * FROM system_usage_daily WHERE start_time >= '%s' AND end_time <='%s' AND type = '%s') AS temp  WHERE avg_cpu_usage >= 0 GROUP BY owt ",StartDate,EndDate,Type)
